...
const gsfComment GsfComment(const struct timespec &when, const char *comment) {
  gsfComment ret = {{when.tv_sec, when.tv_nsec},
                    int(strlen(comment)),
...
void PacketCounts::Verify(const vector<int> &expected) {
  ...
  for (int i = 0; i < NUM_REC_TYPES; ++i) {
    EXPECT_EQ(expected[i], counts_[i]) << "Mismatch. packet #" << i
                                       << " expected: " << expected[i]
                                       << "  found: " << counts_[i] << "\n";
  }
}
...
void VerifyArray(const double *expected, const double *actual, int size) {
  if (size == 0) {
...
std::ostream &operator<<(std::ostream &o, const gsfScaleInfo &scale_info) {
  o << "GsfScaleInfo(" << static_cast<int>(scale_info.compressionFlag) << ", "
    << scale_info.multiplier << ", " << scale_info.offset << ")";
  return o;
}
